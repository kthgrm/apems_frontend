...
import { AwardIcon, Building, Calendar, Trophy } from "lucide-react";
...
export default function Award() {
                    ...
                    try {
                        if (collegeId) {
                            ...
                            if (!validation.valid) {
                                ...
                                // Fetch awards for the validated college and campus
                                const awardRes = await api.get(`/awards`, {
                                    params: {
                                ...
                                setTotalAwards(awardRes.data.data.length);
                            }
                        }
                    } catch (error) {
    ...
    const handleArchived = (id: number | string) => {
        setAwards(prev => prev.filter(p => String(p.id) !== String(id)));
        setTotalAwards(prev => Math.max(0, prev - 1));
    };
                ...
                {isLoading ? (
                    ...
                    invalidSelection ? (
                        ...
                        <div className="flex h-full flex-1 flex-col gap-6 rounded-xl px-10 py-5 overflow-x-auto">
                            <h1 className='text-2xl font-bold'>Awards and Recognition</h1>
                            ...
                            <div className="grid auto-rows-min gap-4 md:grid-cols-3 lg:grid-cols-4">
                                {colleges.map((college) => (
                                    <Card key={college.id} className="hover:shadow-lg transition-shadow duration-200">
                                        <Link to={`/admin/awards-recognition?campus=${campusId}&college=${college.id}`} className="flex flex-col items-center gap-2">
                                            <CardContent>
                        ...
                        <div className="grid auto-rows-min gap-4 md:grid-cols-3 lg:grid-cols-4">
                            {campuses.map((campus) => (
                                <Card key={campus.id} className="hover:shadow-lg transition-shadow duration-200">
                                    <Link to={`/admin/awards-recognition?campus=${campus.id}`}>
