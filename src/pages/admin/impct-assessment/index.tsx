...
export default function ImpactAssessment() {
                    ...
                    try {
                        if (collegeId) {
                            ...
                            if (!validation.valid) {
                                ...
                                // Fetch assessments for the validated college and campus
                                const assessmentRes = await api.get(`/impact-assessments`, {
                                    params: {
                                ...
                                setTotalAssessments(assessmentRes.data.data.length);
                            }
                        }
                    } catch (error) {
    ...
    const handleArchived = (id: number | string) => {
        setAssessments(prev => prev.filter(p => String(p.id) !== String(id)));
                            ...
                            <div className="grid auto-rows-min gap-4 md:grid-cols-3 lg:grid-cols-4">
                                {colleges.map((college) => (
                                    <Card key={college.id} className="hover:shadow-lg transition-shadow duration-200">
                                        <Link to={`/admin/impact-assessment?campus=${campusId}&college=${college.id}`} className="flex flex-col items-center gap-2">
                                            <CardContent>
                        ...
                        <div className="grid auto-rows-min gap-4 md:grid-cols-3 lg:grid-cols-4">
                            {campuses.map((campus) => (
                                <Card key={campus.id} className="hover:shadow-lg transition-shadow duration-200">
                                    <Link to={`/admin/impact-assessment?campus=${campus.id}`}>
