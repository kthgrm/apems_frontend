...
import { Search, Filter, Download, Building2, GraduationCap, Trophy } from 'lucide-react';
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Reports', href: '/admin/reports/awards' },
    { title: 'Awards Report', href: '/admin/reports/awards' },
];

export default function AwardsReport() {
    ...
    const fetchReportData = async () => {
        try {
            ...
            const response = await api.get('/reports/awards', { params });
    ...
    const generatePDF = async () => {
        ...
        try {
            ...
            const url = `${baseURL}/reports/awards/pdf?${params.toString()}`;
    ...
    const { awards, campuses, colleges } = reportData;
                                ...
                                <TableBody>
                                    {awards.data.map((award) => (
                                        <TableRow key={award.id}>
                                            ...
                                            <TableCell>
                                                <Badge variant="outline">{award.awarding_body || 'Not specified'}</Badge>
                                            </TableCell>
                                            <TableCell>
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <div>
                                                        <div className="flex items-center gap-1">
                                                            ...
                                                            <span className="text-xs">{award.college?.campus?.name || 'N/A'}</span>
                                                        </div>
                                                        <div className="flex items-center gap-1">
                                                            <GraduationCap className="h-3 w-3" />
                                                            <span className="text-xs">{award.college?.name || 'N/A'}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </TableCell>
                                            <TableCell>
                                                <p className="text-sm text-muted-foreground">
                                                    {award.location || 'No location specified'}
                                                </p>
                                            </TableCell>
                                            <TableCell>
                                            ...
                                            <TableCell>
                                                <div className="text-sm">
                                                    ...
                                                    <p className="text-muted-foreground">
                                                        by {award.user ? `${award.user.first_name} ${award.user.last_name}` : 'N/A'}
                                                    </p>
                                                </div>
                                            </TableCell>
                                        </TableRow>
                        ...
                        {awards.last_page > 1 && (
                            <div className="flex items-center justify-between px-2 py-4">
                                ...
                                <div className="flex items-center space-x-2">
                                    {awards.current_page > 1 && (
                                        <Button
                                            variant="outline"
                                            size="sm"
                                            onClick={() => handlePageChange(awards.current_page - 1)}
                                        >
                                            Previous
                                        </Button>
                                    )}
                                    {awards.current_page < awards.last_page && (
                                        <Button
                                            variant="outline"
                                            size="sm"
                                            onClick={() => handlePageChange(awards.current_page + 1)}
