...
export const columns = (onArchived?: (id: number | string) => void): ColumnDef<ImpactAssessment>[] => [
    ...
    },
    {
        ...
        cell: ({ row }) => {
            const assessment = row.original
            return (
                <Badge className={`capitalize ${assessment.status === 'approved' ? 'bg-green-200 text-green-800' : assessment.status === 'pending' ? 'bg-yellow-200 text-yellow-600' : 'bg-red-200 text-red-800'}`}>
                    {assessment.status}
                </Badge>
            )
        },
